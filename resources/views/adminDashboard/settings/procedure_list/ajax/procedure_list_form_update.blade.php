{{-- 700021133 - update form --}}
 @if($permissions["update"]) 
<div class="modal-header">
  <h5>{{__("public.update_form_field")}}</h5>
</div> <!--modal-header-->
...
@php
    ...
    $value_category_procedure_id = $records->category_procedure_id;
    ...
    <div class="col-lg-12">
     ...
     <select class="form-control select2 "  name="category_procedure_id" id="category_procedure_id" >
       ...
       @if($data_category_procedure->count() >= 1)
         ...
         <option @if($row->id == $value_category_procedure_id) selected @endif value="{{ encrypt($row->id) }}">{{ $row->name }}</option>
         @endforeach
       @endif
      </select>
    </div>
...
<div class="modal-footer" id="modal-footer">     
   <button   type="button"  id = "Save" name = "Save"  class="btn   btn btn-primary"    onclick="ConfirmUpdateData_procedure_list('procedure_list')"   > <i class="loader fa  "></i>  {{  __("public.save") }}   </button>
...
<input type="hidden" id="id" name="id" value="{{ encrypt($records->id) }}" />
<script>
$('.date').datepicker({
    format: 'yyyy-mm-dd',
  todayBtn: 'linked',
