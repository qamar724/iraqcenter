...
<div class="card-header  d-flex justify-content-between align-items-center">
  <h3 class="card-title "> {{ __("settings.submenu_genders") }} </h3>
  @if($permissions["insertNew"]) 
    <button   type="button" data-bs-toggle="modal"    data-bs-target=".bd-dialog-modal-lg" data-toggle="modal" data-target=".bd-dialog-modal-lg" id = "AddForm" name = "AddForm"  class="btn btn btn-primary btn-sm "    onclick="AddForm_genders()"> <i class="loader fa  "></i>  {{ __("public.add_new_field") }}  </button> 
  @endif
</div>
<div class="card-body">
...
<div class="card border">
    <div class="card-body pb-0">
        ...
        <li class="list-group-item d-flex px-0 justify-content-between">     
         <strong> {{ __("settings.settings_genders_id") }}  </strong>  
              <span class="mb-0">  {{ $counter++ }}</span> 
        </li>     
        <li class="list-group-item d-flex px-0 justify-content-between">     
         <strong> {{ __("settings.settings_genders_name_en") }}  </strong>  
              <span class="mb-0">    {{ $record->name_en }}</span>  
        </li>     
        <li class="list-group-item d-flex px-0 justify-content-between">     
         <strong> {{ __("settings.settings_genders_name_ar") }}  </strong>  
...
<div class="card-footer pt-0 pb-0 text-center card-line-top">
    <div class="row">
        <div class="col-6 pt-3 pb-3 border-right">
          @if($permissions["update"])
           <a href="javascript:void(0)"   data-bs-toggle="modal"    data-bs-target=".bd-dialog-modal-lg" data-toggle="modal" data-target=".bd-dialog-modal-lg" onclick="update_form_genders( '{{$record_id}}'  )"  class="btn btn-primary shadow btn-xs sharp mr-1"><i class="fas fa-pencil-alt"></i></a> 
         @endif
        </div>
        <div class="col-6 pt-3 pb-3 border-right">
          @if($permissions["delete"])
          <a href="javascript:void(0)"   data-bs-toggle="modal"    data-bs-target=".bd-dialog-modal-lg" data-toggle="modal" data-target=".bd-dialog-modal-lg" onclick="delete_form_genders( '{{$record_id}}'  )"  class="btn btn-danger shadow btn-xs sharp "><i class="fa fa-trash"></i></a> 
         @endif
        </div>
  ...
  @else 
    ...
    @if(count($records)  >= 1)
       <div class="table-responsive"  id="Table_Responsive" >
        ...
        <thead>  
        <tr> 
        <th> {{ __("settings.settings_genders_id") }} </th>  
        <th> {{ __("settings.settings_genders_name_en") }} </th>  
        <th> {{ __("settings.settings_genders_name_ar") }} </th>  
 ...
 @if($permissions["viewPage"])
   ...
   <script src=   {{asset("model-js/genders.js")}}></script>
@endif
@endsection  
