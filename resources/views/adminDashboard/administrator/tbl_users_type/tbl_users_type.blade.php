...
<div class="card-header  d-flex justify-content-between align-items-center">
  <h3 class="card-title "> {{ __("administrator.submenu_tbl_users_type") }} </h3>
  @if($permissions["insertNew"]) 
    <button   type="button" data-bs-toggle="modal"    data-bs-target=".bd-dialog-modal-lg" data-toggle="modal" data-target=".bd-dialog-modal-lg" id = "AddForm" name = "AddForm"  class="btn btn btn-primary btn-sm "    onclick="AddForm_tbl_users_type()"> <i class="loader fa  "></i>  {{ __("public.add_new_field") }}  </button> 
  @endif
</div>
<div class="card-body">
...
<div class="card border">
    <div class="card-body pb-0">
        ...
        <li class="list-group-item d-flex px-0 justify-content-between">     
         <strong> {{ __("administrator.administrator_tbl_users_type_id") }}  </strong>  
              <span class="mb-0">  {{ $counter++ }}</span> 
        </li>     
        <li class="list-group-item d-flex px-0 justify-content-between">     
         <strong> {{ __("administrator.administrator_tbl_users_type_name") }}  </strong>  
...
<div class="card-footer pt-0 pb-0 text-center card-line-top">
    <div class="row">
        <div class="col-4 pt-3 pb-3 border-right">
              @if($permissions["update"] && $record->id > 1)
               <a href="javascript:void(0)"   data-bs-toggle="modal"  title="{{ __("public.tooltip_update") }}" tabindex="0" data-plugin="tippy" data-tippy-placement="top" data-tippy="" data-original-title="{{ __("public.tooltip_update") }}"    data-bs-target=".bd-dialog-modal-lg"  data-toggle="modal" data-target=".bd-dialog-modal-lg" onclick="update_form_tbl_users_type('{{$record_id}}','tbl_users_type')"  class="btn btn-primary shadow btn-xs sharp mr-1"><i class="fas fa-pencil-alt"></i></a> 
              @endif
        </div>
        @if($record->id > 1 )
        <div class="col-4 pt-3 pb-3 border-right">
          @if($permissions["delete"])
          <a href="javascript:void(0)"   data-bs-toggle="modal"    data-bs-target=".bd-dialog-modal-lg" data-toggle="modal" data-target=".bd-dialog-modal-lg" onclick="delete_form_tbl_users_type( '{{$record_id}}'  )"  class="btn btn-danger shadow btn-xs sharp "><i class="fa fa-trash"></i></a> 
         @endif
        </div>
        <div class="col-4 pt-3 pb-3 border-right">
               <a href="javascript:void(0)"   data-bs-toggle="modal"  title="{{ __("public.tooltip_set_permission") }}" data-plugin="tippy" data-tippy-placement="top" data-tippy="" data-original-title="{{ __("public.tooltip_set_permission") }}"  data-bs-target=".bd-dialog-modal-lg"  data-toggle="modal" data-target=".bd-dialog-modal-lg" onclick="Permission_form_tbl_users_type('{{$record_id}}','tbl_users_type')"  class="btn btn-info shadow btn-xs sharp "><i class="fa fa-eye"></i></a> 
        </div>
         @else
        <div class="col-4 pt-3 pb-3 border-right"> {{__("public.Full Permission")}} </div>
         @endif
  ...
  @else 
    ...
    @if(count($records)  >= 1)
       <div class="table-responsive"  id="Table_Responsive" >
        ...
        <thead>  
        <tr> 
        <th> {{ __("administrator.administrator_tbl_users_type_id") }} </th>  
        <th> {{ __("administrator.administrator_tbl_users_type_name") }} </th>  
 ...
 @if($permissions["viewPage"])
   ...
   <script src=   {{asset("model-js/tbl_users_type.js")}}></script>
@endif
@endsection  
