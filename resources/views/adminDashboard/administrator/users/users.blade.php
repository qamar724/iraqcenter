...
<div class="card-header  d-flex justify-content-between align-items-center">
  <h3 class="card-title "> {{ __("administrator.submenu_users") }} </h3>
  @if($permissions["insertNew"]) 
    <button   type="button" data-bs-toggle="modal"    data-bs-target=".bd-dialog-modal-lg" data-toggle="modal" data-target=".bd-dialog-modal-lg" id = "AddForm" name = "AddForm"  class="btn btn btn-primary btn-sm "    onclick="AddForm_users()"> <i class="loader fa  "></i>  {{ __("public.add_new_field") }}  </button> 
  @endif
</div>
<div class="card-body">
<!-- start filters code--> 
...
<div class="col-lg-3"> 
 ...
 <select class="form-control select2 "  name="key_tbl_users_type_id" id="key_tbl_users_type_id" >
  <option value="{{ encrypt(-1) }}">{{__("public.select")}}</option>
   @if($data_tbl_users_type->count() >= 1)
     @foreach($data_tbl_users_type as $row)
     <option value="{{ encrypt($row->id) }}" {{ request()->has("key_tbl_users_type_id") && decrypt(request()->input("key_tbl_users_type_id")) == $row->id ? "selected" : "" }}>   {{ $row->name }}  </option>
   @endforeach
   @endif
   </select>
</div>
<div class="col-lg-3"> 
 ...
 <select class="form-control select2 "  name="key_genders_id" id="key_genders_id" >
  <option value="{{ encrypt(-1) }}">{{__("public.select")}}</option>
   @if($data_genders->count() >= 1)
 @php $filter_name="name_$lang" @endphp
     @foreach($data_genders as $row)
     <option value="{{ encrypt($row->id) }}" {{ request()->has("key_genders_id") && decrypt(request()->input("key_genders_id")) == $row->id ? "selected" : "" }}>   {{ $row->$filter_name}}  </option>
   @endforeach
   @endif
   </select>
</div>
...
<div class="card border">
    <div class="card-body pb-0">
        ...
        <li class="list-group-item d-flex px-0 justify-content-between">     
         <strong> {{ __("administrator.administrator_users_hospitals_id_name") }}  </strong>  
...
<div class="card-footer pt-0 pb-0 text-center card-line-top">
    <div class="row">
        <div class="col-3 pt-3 pb-3 border-right">
          @if($permissions["update"])
           <a href="javascript:void(0)"   data-bs-toggle="modal"    data-bs-target=".bd-dialog-modal-lg" data-toggle="modal" data-target=".bd-dialog-modal-lg" onclick="update_form_users( '{{$record_id}}'  )"  class="btn btn-primary shadow btn-xs sharp mr-1"><i class="fas fa-pencil-alt"></i></a> 
         @endif
        </div>
        @if($record["tbl_users_type_id"] != Auth::user()->tbl_users_type_id)
        <div class="col-3 pt-3 pb-3 border-right">
          @if($permissions["delete"])
          <a href="javascript:void(0)"   data-bs-toggle="modal"    data-bs-target=".bd-dialog-modal-lg" data-toggle="modal" data-target=".bd-dialog-modal-lg" onclick="delete_form_users( '{{$record_id}}'  )"  class="btn btn-danger shadow btn-xs sharp "><i class="fa fa-trash"></i></a> 
         @endif
        </div>
        <div class="col-3 pt-3 pb-3 border-right">
  ...
  @else 
    ...
    @if(count($records)  >= 1)
       <div class="table-responsive"  id="Table_Responsive" >
        ...
        <th> {{ __("administrator.administrator_users_hospitals_id_name") }} </th>  
 ...
 @if($permissions["viewPage"])
   ...
   <script src=   {{asset("model-js/users.js")}}></script>
@endif
@endsection  
