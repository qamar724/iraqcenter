       ...
       @if($data_status->count() >= 1)
        @php  $status_id_name="name_$lang" @endphp
         @foreach($data_status as $row)
           <option value="{{ encrypt($row->id) }}">{{ $row->$status_id_name }}</option>
         @endforeach
       @endif
      </select>
    </div>
...
<div class="modal-footer" id="modal-footer">     
   <button   type="button"  id = "Save" name = "Save"  class="btn   btn btn-primary"    onclick="distributions_ConfirmInsertData()"   > <i class="loader fa  "></i>  {{  __("public.save") }}   </button>
