...
/**************************************************************************/
/*************Begin Functionality to table procedure_list ******************/
/*************************************************************************/



// start add form button
function AddForm_procedure_list() {
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajax({
           url: "procedure_list_form_add",
        method: "get",
...
function update_form_procedure_list(id, table) { 
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
    $.ajax({
        url: "procedure_list_form_update",
        method: "post",
        data: {
            id: id,
            table: table,
...
function ConfirmUpdateData_procedure_list(table) {
    var errorList = [];
    var form_data = new FormData();
    var outputErr=[];
    form_data.append("table", "procedure_list"  );
    form_data.append("id", $("#id").val()  );
    form_data.append("category_procedure_id",$("#category_procedure_id").val());


    console.log($("#category_procedure_id").val());
...
//Confirm insert data button start 
  function  procedure_list_ConfirmInsertData() {
  // PROCCESS 7000001
    var errorList = [];
    var outputErr = [];
    var form_data = new FormData();
...
//delete from table procedure_list start 
function delete_form_procedure_list(id) {
        ...
        $.get("procedure_list_delete", {
          ...
          table: table,
            ...
            if (data.response == "deleted") {
                ...
                $(".result_content").html("<div class='alert alert-danger bg-danger text-white border-0 m-0'><h3>"+data.message+"</h3> <br> <input type='button'  data-toggle='modal' data-target='.bd-dialog-modal-lg' value='"+btn_close+"' data-bs-dismiss='modal'   onclick='hidepopup()' class='btn btn-light'></div>");
...
//confirm delete from table  procedure_list end
/**************************************************************************/
/*************End Functionality to table procedure_list ******************/
/*************************************************************************/
