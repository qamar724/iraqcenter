...
function update_form_hospitals(id, table) { 
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
    $.ajax({
        url: "hospitals_form_update",
        method: "post",
        data: {
            id: id,
            table: table,
...
function ConfirmUpdateData_hospitals(table) {
    var errorList = [];
    var form_data = new FormData();
    var outputErr=[];
    form_data.append("table", "hospitals"  );
    ...
    if ($("#city_id").val() == -1) {
      errorList.push("Err_city_id");
      $("#city_id").next().toggleClass("border_red", $("#city_id :selected").val() == -1);
    } else {
      $("#city_id").next().toggleClass("border_green", $("#city_id :selected").val() > 0);
      errorList.splice(errorList.indexOf("Err_city_id"), -1)
...
//Confirm insert data button start 
  function  hospitals_ConfirmInsertData() {
  // PROCCESS 7000001
    var errorList = [];
    var outputErr = [];
    var form_data = new FormData();
...
//delete from table hospitals start 
function delete_form_hospitals(id) {
...
//confirm delete from table  hospitals end
/**************************************************************************/
/*************End Functionality to table hospitals ******************/
/*************************************************************************/
