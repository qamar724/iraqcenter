...
/**************************************************************************/
/*************Begin Functionality to table monthly_evaluation_forms ******************/
/*************************************************************************/



// start add form button
function AddForm_monthly_evaluation_forms() {
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajax({
           url: "monthly_evaluation_forms_form_add",
        method: "get",
...
function update_form_monthly_evaluation_forms(id, table) { 
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
    $.ajax({
        url: "monthly_evaluation_forms_form_update",
        method: "post",
        data: {
            id: id,
            table: table,
...
function ConfirmUpdateData_monthly_evaluation_forms(table) {
    var errorList = [];
    var form_data = new FormData();
    var outputErr=[];
    form_data.append("table", "monthly_evaluation_forms"  );
...
if(outputErr.length >= 1){
  ...
  if (errorList.length == 0) {
     ...
     $("#Close").prop("disabled", true);
           $.ajax({ 
                   url: "monthly_evaluation_forms_update",
...
//Confirm insert data button start 
  function  monthly_evaluation_forms_ConfirmInsertData() {
  // PROCCESS 7000001
    var errorList = [];
    var outputErr = [];
    var form_data = new FormData();
 ...
 $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
  if (errorList.length == 0) {
     ...
     $(".loader").addClass(" fa-spinner fa-spin");
           $.ajax({ 
                   url: "monthly_evaluation_forms_insert",
              dataType: "text",  // what to expect back from the PHP script, if anything
                 cache: false,
           contentType: false,
           processData: false,
...
//delete from table monthly_evaluation_forms start 
function delete_form_monthly_evaluation_forms(id) {
    ...
    var output = "";
    output += "<div class='alert alert-danger bg-danger text-white border-0 m-0'><h3> "+msg+"</h3> <div class='modal-footer' id='modal-footer'  > <input type='button'    value='"+btn_yes+" '  id='confirmDelete' onclick='ConfirmDelete_monthly_evaluation_forms(\""+id+"\")'   class='btn btn-success'><input type= 'button'  data-bs-toggle='modal'    data-bs-target='.bd-dialog-modal-lg' data-toggle='modal' data-target='.bd-dialog-modal-lg'  data-bs-dismiss='modal' value='"+btn_no+" ' onclick='hidepopup()' class='btn btn-warning'></div></div>";
    $(".result_content").html(output)
}
// delete from table  monthly_evaluation_forms end


//confirm delete from table  monthly_evaluation_forms end
function ConfirmDelete_monthly_evaluation_forms(id) {
    var table = $("#hiddenTable").val();
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup'   />");
    var currentLang = getCookie("lang");
    ...
    if(currentLang == "ar"){
      ...
      btn_close = "Close";
        ...
        $.get("monthly_evaluation_forms_delete", {
             id: id,
          table: table,
        }, function (data) {
...
//confirm delete from table  monthly_evaluation_forms end
/**************************************************************************/
/*************End Functionality to table monthly_evaluation_forms ******************/
/*************************************************************************/
