...
/**************************************************************************/
/*************Begin Functionality to table assessment_criteria ******************/
/*************************************************************************/



// start add form button
function AddForm_assessment_criteria() {
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajax({
           url: "assessment_criteria_form_add",
        method: "get",
...
function update_form_assessment_criteria(id, table) { 
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
    $.ajax({
        url: "assessment_criteria_form_update",
        method: "post",
        data: {
            id: id,
            table: table,
...
function ConfirmUpdateData_assessment_criteria(table) {
    var errorList = [];
    var form_data = new FormData();
    var outputErr=[];
    form_data.append("table", "assessment_criteria"  );
    form_data.append("id", $("#id").val()  );


    /*********** start required fields ***********/
    if ($("#monthly_evaluation_forms_id").val() == -1) {
      errorList.push("Err_monthly_evaluation_forms_id");
      $("#monthly_evaluation_forms_id").next().toggleClass("border_red", $("#monthly_evaluation_forms_id :selected").val() == -1);
    } else {
      $("#monthly_evaluation_forms_id").next().toggleClass("border_green", $("#monthly_evaluation_forms_id :selected").val() > 0);
      errorList.splice(errorList.indexOf("Err_monthly_evaluation_forms_id"), -1)
      form_data.append("monthly_evaluation_forms_id", $("#monthly_evaluation_forms_id").val());
    }
    if ($("#question").val() == "") {
      errorList.push("Err_question");
      $("#question").css("border", "1px solid red");
    } else {
      $("#question").css("border", "1px solid green");
      errorList.splice(errorList.indexOf("Err_question"), -1)
...
if(outputErr.length >= 1){
  ...
  if (errorList.length == 0) {
     ...
     $("#Close").prop("disabled", true);
           $.ajax({ 
                   url: "assessment_criteria_update",
...
//Confirm insert data button start 
  function  assessment_criteria_ConfirmInsertData() {
  // PROCCESS 7000001
    var errorList = [];
    var outputErr = [];
    var form_data = new FormData();
 ...
 $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
  if (errorList.length == 0) {
     ...
     $(".loader").addClass(" fa-spinner fa-spin");
           $.ajax({ 
                   url: "assessment_criteria_insert",
              dataType: "text",  // what to expect back from the PHP script, if anything
                 cache: false,
           contentType: false,
           processData: false,
...
//delete from table assessment_criteria start 
function delete_form_assessment_criteria(id) {
    ...
    var output = "";
    output += "<div class='alert alert-danger bg-danger text-white border-0 m-0'><h3> "+msg+"</h3> <div class='modal-footer' id='modal-footer'  > <input type='button'    value='"+btn_yes+" '  id='confirmDelete' onclick='ConfirmDelete_assessment_criteria(\""+id+"\")'   class='btn btn-success'><input type= 'button'  data-bs-toggle='modal'    data-bs-target='.bd-dialog-modal-lg' data-toggle='modal' data-target='.bd-dialog-modal-lg'  data-bs-dismiss='modal' value='"+btn_no+" ' onclick='hidepopup()' class='btn btn-warning'></div></div>";
    $(".result_content").html(output)
}
// delete from table  assessment_criteria end


//confirm delete from table  assessment_criteria end
function ConfirmDelete_assessment_criteria(id) {
    var table = $("#hiddenTable").val();
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup'   />");
    var currentLang = getCookie("lang");
    ...
    if(currentLang == "ar"){
      ...
      btn_close = "Close";
        ...
        $.get("assessment_criteria_delete", {
             id: id,
          table: table,
        }, function (data) {
...
//confirm delete from table  assessment_criteria end
/**************************************************************************/
/*************End Functionality to table assessment_criteria ******************/
/*************************************************************************/
