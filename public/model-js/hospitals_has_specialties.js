...
/**************************************************************************/
/*************Begin Functionality to table hospitals_has_specialties ******************/
/*************************************************************************/



// start add form button
function AddForm_hospitals_has_specialties() {
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajax({
           url: "hospitals_has_specialties_form_add",
        method: "get",
...
function update_form_hospitals_has_specialties(id, table) { 
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
    $.ajax({
        url: "hospitals_has_specialties_form_update",
        method: "post",
        data: {
            id: id,
            table: table,
...
function ConfirmUpdateData_hospitals_has_specialties(table) {
    var errorList = [];
    var form_data = new FormData();
    var outputErr=[];
    form_data.append("table", "hospitals_has_specialties"  );
    ...
    if ($("#specialties_id").val() == -1) {
      errorList.push("Err_specialties_id");
      $("#specialties_id").next().toggleClass("border_red", $("#specialties_id :selected").val() == -1);
    } else {
      $("#specialties_id").next().toggleClass("border_green", $("#specialties_id :selected").val() > 0);
      errorList.splice(errorList.indexOf("Err_specialties_id"), -1)
...
if(outputErr.length >= 1){
  ...
  if (errorList.length == 0) {
     ...
     $("#Close").prop("disabled", true);
           $.ajax({ 
                   url: "hospitals_has_specialties_update",
...
//Confirm insert data button start 
  function  hospitals_has_specialties_ConfirmInsertData() {
  // PROCCESS 7000001
    var errorList = [];
    var outputErr = [];
    var form_data = new FormData();
 ...
 $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
  if (errorList.length == 0) {
     ...
     $(".loader").addClass(" fa-spinner fa-spin");
           $.ajax({ 
                   url: "hospitals_has_specialties_insert",
              dataType: "text",  // what to expect back from the PHP script, if anything
                 cache: false,
           contentType: false,
           processData: false,
...
//delete from table hospitals_has_specialties start 
function delete_form_hospitals_has_specialties(id) {
    ...
    var output = "";
    output += "<div class='alert alert-danger bg-danger text-white border-0 m-0'><h3> "+msg+"</h3> <div class='modal-footer' id='modal-footer'  > <input type='button'    value='"+btn_yes+" '  id='confirmDelete' onclick='ConfirmDelete_hospitals_has_specialties(\""+id+"\")'   class='btn btn-success'><input type= 'button'  data-bs-toggle='modal'    data-bs-target='.bd-dialog-modal-lg' data-toggle='modal' data-target='.bd-dialog-modal-lg'  data-bs-dismiss='modal' value='"+btn_no+" ' onclick='hidepopup()' class='btn btn-warning'></div></div>";
    $(".result_content").html(output)
}
// delete from table  hospitals_has_specialties end


//confirm delete from table  hospitals_has_specialties end
function ConfirmDelete_hospitals_has_specialties(id) {
    var table = $("#hiddenTable").val();
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup'   />");
    var currentLang = getCookie("lang");
    ...
    if(currentLang == "ar"){
      ...
      btn_close = "Close";
        ...
        $.get("hospitals_has_specialties_delete", {
             id: id,
          table: table,
        }, function (data) {
...
//confirm delete from table  hospitals_has_specialties end
/**************************************************************************/
/*************End Functionality to table hospitals_has_specialties ******************/
/*************************************************************************/
