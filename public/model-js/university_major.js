...
/**************************************************************************/
/*************Begin Functionality to table university_major ******************/
/*************************************************************************/



// start add form button
function AddForm_university_major() {
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajax({
           url: "university_major_form_add",
        method: "get",
...
function update_form_university_major(id, table) { 
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup' />");
    $.ajaxSetup({ headers: {  "X-CSRF-TOKEN": $("#token").val()} });
    $.ajax({
        url: "university_major_form_update",
        method: "post",
        data: {
            id: id,
            table: table,
...
function ConfirmUpdateData_university_major(table) {
    var errorList = [];
    var form_data = new FormData();
    var outputErr=[];
    form_data.append("table", "university_major"  );
    form_data.append("id", $("#id").val()  );


    /*********** start required fields ***********/
    if ($("#name_en").val() == "") {
      errorList.push("Err_name_en");
      $("#name_en").css("border", "1px solid red");
    } else {
      $("#name_en").css("border", "1px solid green");
      errorList.splice(errorList.indexOf("Err_name_en"), -1)
      form_data.append("name_en", $("#name_en").val());
    }
    if ($("#name_ar").val() == "") {
      errorList.push("Err_name_ar");
      $("#name_ar").css("border", "1px solid red");
    } else {
      $("#name_ar").css("border", "1px solid green");
      errorList.splice(errorList.indexOf("Err_name_ar"), -1)
      form_data.append("name_ar", $("#name_ar").val());
    }
    /*********** end required fields ***********/
...
//Confirm insert data button start 
  function  university_major_ConfirmInsertData() {
  // PROCCESS 7000001
    var errorList = [];
    var outputErr = [];
    var form_data = new FormData();
...
//delete from table university_major start 
function delete_form_university_major(id) {
    ...
    var output = "";
    output += "<div class='alert alert-danger bg-danger text-white border-0 m-0'><h3> "+msg+"</h3> <div class='modal-footer' id='modal-footer'  > <input type='button'    value='"+btn_yes+" '  id='confirmDelete' onclick='ConfirmDelete_university_major(\""+id+"\")'   class='btn btn-success'><input type= 'button'  data-bs-toggle='modal'    data-bs-target='.bd-dialog-modal-lg' data-toggle='modal' data-target='.bd-dialog-modal-lg'  data-bs-dismiss='modal' value='"+btn_no+" ' onclick='hidepopup()' class='btn btn-warning'></div></div>";
    $(".result_content").html(output)
}
// delete from table  university_major end


//confirm delete from table  university_major end
function ConfirmDelete_university_major(id) {
    var table = $("#hiddenTable").val();
    $(".result").show();
    $(".result_content").html("<img   src='../images/loading.gif'  class='loaderImgPopup'   />");
    var currentLang = getCookie("lang");
...
//confirm delete from table  university_major end
/**************************************************************************/
/*************End Functionality to table university_major ******************/
/*************************************************************************/
