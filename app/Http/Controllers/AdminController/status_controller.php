...
if (count($request->input()) >=1) {
 ...
 /********************************************************************/
 $data=$data->where("status.is_deleted","=",0);// 0 not deleted & 1 is deleted
if (Auth::user()["tbl_users_type_id"] ==1) { // super admin
  ...
  if (isset($_COOKIE["status_pagination"])) {
    $pagination = $_COOKIE["status_pagination"];
  }
  
 $data=$data->paginate($pagination);
 $layout ="table";
  if (isset($_COOKIE["status"])) { 
    $layout = $_COOKIE["status"];
  }
    ...
    return view("adminDashboard.settings.status.status" ,$array);
  }
function  status_delete(Request $request){
  //  proccess 10000008 - delete controller
   if (Auth::user()["tbl_users_type_id"] ==1) {  // Super admin
   $permissions = array( "delete"=>true   );
   }else{
   $permissions = array( "delete"=> Permission::CheckPermission(4,"status")); 
  }
...
$deleted = status_model::where("id", $id)->update(["is_deleted" => 1]);
    if($deleted){                    
    activity("status") 
    ->performedOn($status) 
    ->causedBy(Auth::user()) 
    ->withProperties([ 
      "attributes" => json_encode($status, JSON_UNESCAPED_UNICODE), 
    ]) 
    ->log("deleted"); 
...
function  status_form_add(){
  //  proccess 100000057 -add form
   ...
   if (Auth::user()["tbl_users_type_id"] ==1) {  // Super admin
    ...
    $permissions = array( "insertNew"=> Permission::CheckPermission(3,"status")); 
   }
   ...
   $array = array(
      ...
      "lang"=>$lang,
       ...
       return view("adminDashboard.settings.status.ajax.status_form_add",$array);
  }
function  status_insert(Request $request){
  //  proccess 10000004 - confirm insert
   $request->validate(status_model::rules($request)); 
   $data=array(); 
   $data["name_en"]=  $request->input()["name_en"] ; 
   $data["name_ar"]=  $request->input()["name_ar"] ; 
$data["created_by"]=Auth::user()->id;
$insert = status_model::create($data);
if($insert){
...
function status_form_update(Request $request){
  // proccess 100000013 - update form
$id =decrypt($request->input("id"));
$records = status_model::findOrFail($id);
$lang =app()->getLocale();
   if (Auth::user()["tbl_users_type_id"] ==1) {  // Super admin
      $permissions = array("update"=>true );
   }else{
      $permissions = array( "update"=> Permission::CheckPermission(2,"status") ); 
   }
...
$array = array(
    ...
    return view("adminDashboard.settings.status.ajax.status_form_update",$array);
}
function  status_update(Request $request){
  // proccess 1000000266
  $settings_status = status_model::findOrFail(  decrypt($request->input("id")) );
  $oldData = $settings_status->getOriginal();
  //$request->validate(status_model::rules());;
  $data = array();
  $data["name_en"]= $request->input()["name_en"] ; 
  $data["name_ar"]= $request->input()["name_ar"] ; 
  $data["updated_by"]=Auth::user()["id"];
  $data["updated_at"]=date("Y-m-d H:i:s" );
  $updated = status_model::where("id","=", decrypt($request->input("id")) )->update($data);
  if($updated){ 
  $newData = $data;
  activity("status")
  ->performedOn($settings_status)
  ->causedBy(Auth::user())
  ->withProperties([
      "old" => $oldData,
      "attributes" => $newData,
